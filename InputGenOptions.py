...
List_optional_flights   = tuple(wb["Optional Flight"].iter_rows())
List_new_flights        = tuple(wb["New Flight"].iter_rows())
List_new_itineraries    = tuple(wb["New Itinerary"].iter_rows())


fn_optional_flights = []
for flight in List_optional_flights[1:]:
    if flight[0].value:        
        fn_optional_flights.append(flight[0].value)
...
for flight in List_flights[1:]:
    if flight[0].value:
        ...
        L.append(new_flight)
        if flight[0].value not in fn_optional_flights:
            LF.append(new_flight)

for flight in L:
    if flight.FN in fn_optional_flights:
        LO.append(flight)

for flight in List_new_flights[1:]:
    if flight[0].value:
        ac_cost = {}
        ac_cost[List_flights[0][5].value] = flight[5].value
        ac_cost[List_flights[0][6].value] = flight[6].value
...
for itinerary in List_new_itineraries[1:]:
    if itinerary[1].value:
        legs = []
        legs.append(itinerary[6].value)
        if itinerary[7].value != 0:
...
data = (L,P,N,K,RR,Arcs,Nodes,NGk, Delta, LF, LO, PO)
with open('input_data_options.pickle', 'wb') as file:
